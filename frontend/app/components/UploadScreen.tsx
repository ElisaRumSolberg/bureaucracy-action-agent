...
export default function UploadScreen({ onFileSelected, errorMessage }: Props) {
  ...
  return (
    <div className="relative flex-1 overflow-hidden">
      <div
        aria-hidden
        className="pointer-events-none absolute left-1/2 top-[-15%] h-[360px] w-[600px] -translate-x-1/2 rounded-full bg-brand opacity-[0.1] blur-[110px] dark:opacity-[0.16]"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute right-[10%] top-[10%] h-[220px] w-[220px] rounded-full bg-amber-400 opacity-[0.06] blur-[100px] dark:opacity-[0.08]"
      />

      <div className="relative mx-auto flex w-full max-w-2xl flex-col items-center gap-4 px-6 py-10 text-center">
        <div>
          <span className="mb-1 inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white/70 px-3 py-1 text-[11px] font-semibold uppercase tracking-widest text-brand backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/60 dark:text-indigo-400">
        ...
        <div
          ...
          className={`flex w-full cursor-pointer flex-col items-center gap-1.5 rounded-2xl border-2 border-dashed px-10 py-6 backdrop-blur transition-colors ${
            isDragging
              ? "border-brand bg-brand-light dark:border-indigo-500 dark:bg-indigo-950/30"
              : "border-zinc-300 bg-white/40 hover:border-brand/60 dark:border-zinc-700 dark:bg-zinc-900/40 dark:hover:border-indigo-500/60"
          }`}
        >
          <svg
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-x-2 gap-y-2 text-xs text-zinc-500 dark:text-zinc-400">
          {FLOW_STEPS.map((step, index) => (
            <span key={step} className="flex items-center gap-2">
              <span className="rounded-full border border-zinc-200 bg-white/70 px-3 py-1 font-medium text-zinc-700 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/70 dark:text-zinc-300">
                {step}
              </span>
